...
{
    return [NSTimer timerWithTimeInterval:ti target:[NSTimer class] selector:@selector(executeTimerUserInfoAsBlock:) userInfo:block repeats:yesOrNo];
}

+ (NSTimer *)scheduledTimerWithTimeInterval:(NSTimeInterval)ti usingBlock:(void(^)(NSTimer *timer))block repeats:(BOOL)yesOrNo
{
    return [NSTimer scheduledTimerWithTimeInterval:ti target:[NSTimer class] selector:@selector(executeTimerUserInfoAsBlock:) userInfo:block repeats:yesOrNo];
}
