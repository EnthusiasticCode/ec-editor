...
@interface CodeViewBaseContentView : UIView
@property (nonatomic, weak) CodeViewBase *parentCodeView;
@end


@interface CodeViewBase () {
@private
    CodeViewBaseContentView *_contentView;
...
static void init(CodeViewBase *self)
{
    self.contentMode = UIViewContentModeRedraw;
    self.clearsContextBeforeDrawing = NO;
    self->_contentView = [CodeViewBaseContentView new];
...
- (void)addPassLayerBlock:(TextRendererLayerPass)block underText:(BOOL)isUnderlay forKey:(NSString *)passKey setupTileBlock:(CodeViewBaseTileSetupBlock)setupBlock cleanupTileBlock:(CodeViewBaseTileSetupBlock)cleanupBlock
{
    if (isUnderlay)
    {
        if (!underlayPasses)
...
@implementation CodeViewBaseContentView
...
- (void)drawRect:(CGRect)rect
{
    ...
    [parentCodeView->setupPasses enumerateKeysAndObjectsUsingBlock:^(id key, id obj, BOOL *stop) {
        ((CodeViewBaseTileSetupBlock)obj)(context, rect);
    }];
