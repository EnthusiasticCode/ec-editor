...
- (void)_scopeAdded:(TMScope *)scope;
- (void)_scopeRemoved:(TMScope *)scope;
...
- (void)reparseWithUnsavedContent:(NSString *)content {
  ...
  for (Change *change in changes) {
    ...
    [_rootScope shiftByReplacingRange:oldRange withRange:newRange onRemove:^(TMScope *scope){
      [self _scopeRemoved:scope];
    }];
  ...
  while (unparsedRanges.count) {
    ...
    [_rootScope removeChildScopesInRange:linesRange onRemove:^(TMScope *scope){
      [self _scopeRemoved:scope];
    }];
...
#warning TODO: fix this to only account for the parts of the captures scope that aren't captures themselves
  ...
  for (NSUInteger currentMatchRangeIndex = 1; currentMatchRangeIndex < numMatchRanges; ++currentMatchRangeIndex) {
    ...
    [self _scopeAdded:currentCaptureScope];
