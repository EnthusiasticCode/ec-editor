...
@protocol ECJumpBarViewDelegate <NSObject>

/// Called when the user tap a button in the button stack.
- (void)jumpBarView:(ECJumpBarView *)jumpBarView tapAtStackIndex:(NSUInteger)index;
...
- (void)jumpBarView:(ECJumpBarView *)jumpBarView didPopButtonAtStackIndex:(NSUInteger)index;

/// Called when the editable search string is changed.
- (void)jumpBarView:(ECJumpBarView *)jumpBarView changedSearchStringTo:(NSString *)searchString;

@end

@interface ECJumpBarView : UIView

@property (nonatomic, assign) id<ECJumpBarViewDelegate> delegate;
